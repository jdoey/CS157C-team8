...
import { Flex, Box, Text, Button, VStack, HStack, Badge } from "@chakra-ui/react";
...
function DogFriendlyPlaces() {
  ...
  const fetchDogFriendlyPlaces = (latitude, longitude) => {
    axios.post('http://localhost:3001/Places/dog-friendly-places', { latitude, longitude })
      .then(response => {
  ...
  const handleLocation = () => {
    navigator.geolocation.getCurrentPosition(
      position => {
        fetchDogFriendlyPlaces(position.coords.latitude, position.coords.longitude);
      },
      () => {
  ...
  return (
    <Flex direction="column" align="center" justify="center" p={4}>
      <Button onClick={handleLocation} colorScheme="blue">Find Dog-Friendly Places</Button>
      {error && <Text color="red.500">{error}</Text>}
      <Box mt={4}>
        {places.length > 0 ? (
          places.map((place, index) => (
            <Box key={index} p={4} shadow="md" borderWidth="1px" mb={2} borderRadius="lg">
              <Text fontWeight="bold" fontSize="xl">{place.name}</Text>
              ...
              <Box mt={2}>
                ...
                {place.events.map((event, eIndex) => (
                  <Text key={eIndex} fontSize="sm">{event.title} - {event.date}</Text>
                ))}
              </Box>
            </Box>
          ))
